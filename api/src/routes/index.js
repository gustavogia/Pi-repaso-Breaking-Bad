...
router.post('/character', async (req, res) => {
    ...
    if(!name || !nickname || !birthday || !image || !status || !occupation) res.status(400).json({msg: "Faltan datos"})
    try{
    ...
    res.send(`El personaje ${name} fue creado con exito`);
    }
    catch (error) {res.send (`El personaje ${name} ya existe`)}
})
